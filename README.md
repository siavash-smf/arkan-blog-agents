...
هر سه از یک هسته‌ی مشترک استفاده می‌کنند: ثبت زنده‌ی گام‌ها، حلقه‌ی نویسنده ⇄ ویراستار، چک‌های قطعی، و حافظه‌ی درس‌ها.
...
├── app/
│   ├── studio/               ← داشبورد: خط تولید + شبکه‌های اجتماعی + پست‌ها + درس‌ها
│   ├── blog/                 ← بلاگ عمومی + صفحه‌ی مقاله (متا + JSON-LD)
│   └── api/
│       ├── pipeline/…        ← اجرای پایپ‌لاین + وضعیت زنده (هر دو نوع اجرا)
│       ├── social/…          ← بازآفرینی + فهرست و تأیید محتوای اجتماعی
...
└── supabase/schema.sql       ← اسکیمای ۵ جدول
```
